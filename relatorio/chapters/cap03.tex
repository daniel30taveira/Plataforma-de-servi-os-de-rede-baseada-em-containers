...
Para suportar a plataforma e garantir o isolamento do \textit{host} físico, a infraestrutura base será implementada em ambiente virtualizado. 

\begin{itemize}
    \item \textbf{Virtualização (VMware):} A plataforma escolhida para o \textit{hypervisor} local é o VMware (Workstation/Player). Esta escolha justifica-se pela sua estabilidade, excelente gestão de redes virtuais (Bridged/NAT) necessárias para simular o tráfego externo/interno e eficiência na alocação de recursos.
    \item \textbf{Sistema Operativo (\textit{Guest OS}):} Para o sistema operativo que irá alojar os \textit{containers}, optou-se por uma distribuição Linux orientada a servidores, nomeadamente o \textbf{Ubuntu Server 24.04 LTS} (ou alternativamente o \textbf{Debian 12}). Estas distribuições foram selecionadas por possuírem vasto suporte da comunidade, excelente compatibilidade nativa com o Docker/Podman e com o Ansible, e por não incluírem interfaces gráficas (\textit{headless}), o que minimiza a superfície de ataque e o consumo de recursos.
\end{itemize}